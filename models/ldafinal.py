...
names = ['cpu_load', 'cpu_interrupts', 'cpu_switches', 'cpu_uidle', 'cpu_uintr', 'cpu_uiowait', 'cpu_unice', 'cpu_usoftirq', 'cpu_usteal', 'cpu_usystem', 'cpu_uuser', 'cpu_procrun', 'in_ens32', 'in_vibr0_nic', 'in_vibr0', 'out_ens32', 'out_vibr0_nic', 'swap_free', 'inode_pfree', ' vfs_size_free', 'vfs_size_used', 'mem_size', 'class']

class LDA():
	def __init__(self, data, num_dims = 1, convert_data = 0, percentile = 50, threshold = 0, labelcol = -1, split_ratio = 0.9):
		'''
	Class constructor.
	--------------------------------
	...
	def to_categorical(self, data, percentile):
		fraction = percentile / 100.0   # partition data based on percentile of label column
	...
	def fit(self):
		...
		def estimate_params(data):
			...
			for c in means.keys():  # for each class
				S_B += np.multiply(len(self.classwise[c]), np.outer((means[c] - overall_mean),  (means[c] - overall_mean)))
			...
			eiglist = [(eigvals[i], eigvecs[:, i], names[i])  for i in range(len(eigvals))]
			# sort the eigvals in decreasing order
			
			eiglist = sorted(eiglist, key = lambda x : x[0], reverse = True)
			eig_pairs = [(np.abs(eigvals[i]), eigvecs[:,i], names[i]) for i in range(len(eigvals))]
		...
		for c in self.classes:
			...
			rows = random.sample(list(self.classwise[c].index), int(self.classwise[c].shape[0] * self.split_ratio))
	...
	def calculate_score_gaussian(self, data):
		...
		likelihoods = np.array([[self.priors[c] * self.pdf([x[ind] for ind in range(len(x))], self.gaussian_means[c], 
								self.gaussian_cov[c]) for c in classes] for x in proj])
	...
	def plot_bivariate_gaussians(self):
		...
		for c in self.means.keys():
			...
			surf = ax3D.plot_surface(X, Y, Z, rstride=1, cstride=1, color=plotlabels[c], linewidth=0, antialiased=False)
		plt.show()
	...
	def plot_proj_1D(self, data):
		...
		for i, row in data.iterrows():
			...
			plt.scatter(proj, np.random.normal(0,1,1)+0, color = plotlabels[row[self.labelcol]])
		plt.show()
	...
	def plot_proj_2D(self, data):
		...
		for i, row in data.iterrows():
			proj = np.dot(self.w, row[:self.labelcol])
			plt.scatter(proj[0], proj[1], color = plotlabels[row[self.labelcol]])
		plt.show()
	
if __name__ == '__main__':
	data = pd.read_csv(sys.argv[1])
	labelcol = int(sys.argv[2])
	lda = LDA(data, num_dims=5, convert_data=0, threshold=0, labelcol=labelcol)
